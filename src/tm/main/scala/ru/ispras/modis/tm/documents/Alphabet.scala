...
class Alphabet(private[documents] val indexWordsMap: Map[AttributeType, TIntObjectHashMap[String]],
    ...
    def toSeq = wordsIndexMap.head._2.keys().map(_.asInstanceOf[String]).zip(wordsIndexMap.head._2.values())
}
...
object Alphabet {
     ...
     */
    def apply(wordIndexMap: Map[AttributeType, TObjectIntHashMap[String]]): Alphabet = {
    ...
    def apply(wordIndexSeq: Array[(String, Int)]): Alphabet = {

        val reverseMap = new TObjectIntHashMap[String]()
        wordIndexSeq.foreach({case (k, v) => reverseMap.put(k, v)})

        apply(Map[AttributeType, TObjectIntHashMap[String]](DefaultAttributeType -> reverseMap))
    }
}